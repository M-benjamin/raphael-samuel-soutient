...
function StripePaymentForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/stripe/appointment-payment-intent", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ businessId, appointmentId, amount }),
      });
      ...
      if (paymentIntent?.status === "succeeded") {
        onPaid(paymentIntent.id, amount, "card");
      }
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3">
      ...
      <button
        ...
        className="w-full py-3 rounded-xl font-bold text-white text-[14px] flex items-center justify-center gap-2 transition-opacity"
        style={{
          background: "linear-gradient(135deg,#635bff,#4f46e5)",
          opacity: !stripe || loading || amount <= 0 ? 0.5 : 1,
        }}
      >
        {loading ? (
        ...
        {loading
          ? "Processing…"
          : `Pay $${amount > 0 ? amount.toFixed(2) : "—"} by Card`}
      </button>
    </form>
  );
}
...
export function PaymentModal({
  ...
  const resolveServicePrice = (): number | null => {
    if (!svc) return null;
    if (svc.price_type === "call_for_price") return null;
  ...
  const remaining =
    Number(appointment.amount_remaining ?? 0) > 0
      ? Number(appointment.amount_remaining)
      : displayTotal > 0
        ? Math.max(0, displayTotal - alreadyPaid)
        : 0;

  const isCustomPrice =
    svc?.price_type === "call_for_price" || servicePrice === null;
    ...
    try {
      ...
      const hash = (await window.ethereum!.request({
        ...
        params: [
          {
            from: account,
            to: config.usdc_contract_address,
            data,
            gas: toHex(80000),
          },
        ],
  ...
  const explorerUrl =
    txHash && config ? `https://polygonscan.com/tx/${txHash}` : "";
        ...
        >
          ...
          <div
            ...
            style={{
              ...
              background:
                "linear-gradient(135deg,rgba(20,168,181,0.06),rgba(13,115,119,0.04))",
            }}
          >
              ...
              <div className="space-y-3">
                ...
                >
                  ...
                  <div>
                    <p
                      ...
                      style={{ color: "#0a2e30" }}
                    >
                      Pay {remaining > 0 ? "Remaining" : "Full"} Amount (USDC)
                      {!isCustomPrice &&
                        (remaining > 0 ? remaining : displayTotal) > 0 && (
                          <span
                            className="ml-2 text-[13px] font-semibold"
                            style={{ color: "#0d7377" }}
                          >
                            $
                            {(remaining > 0 ? remaining : displayTotal).toFixed(
                              2,
                            )}
                          </span>
                        )}
                    ...
                    <p className="text-[12px]" style={{ color: "#64748b" }}>
                      Pay the full balance via crypto wallet
                    </p>
                  </div>
                </button>

                <button
                  onClick={() => loadConfig("partial-usdc")}
                  className="w-full flex items-center gap-4 px-4 py-4 rounded-2xl text-left transition-all hover:scale-[1.01]"
                  style={{
                ...
                >
                  ...
                  <div>
                    <p
                      ...
                      style={{ color: "#0a2e30" }}
                    >
                      Pay a Partial Amount (USDC)
                    </p>
                    <p className="text-[12px]" style={{ color: "#64748b" }}>
                      Pay any USDC amount now — remainder settled later
                    </p>
                  </div>
                </button>
                ...
                >
                  ...
                  <Elements stripe={stripePromise}>
                    <StripePaymentForm
                      amount={remaining > 0 ? remaining : displayTotal}
                      appointmentId={appointment.id}
                ...
                <p
                  ...
                  style={{ color: "#94a3b8" }}
                >
                  Cash payments are recorded by the clinic — contact your
                  doctor.
                </p>
              </div>
            )}
              ...
              <div className="space-y-4">
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-2 text-[12px]">
                    ...
                    <span className="font-medium" style={{ color: "#0a2e30" }}>
                      {config?.network_name}
                    </span>
                    ...
                    <span
                      ...
                      style={{ color: "#0a2e30" }}
                    >
                      {config?.receiver_wallet.slice(0, 10)}…
                      {config?.receiver_wallet.slice(-6)}
                    </span>
                  </div>
                </div>
                <div className="flex gap-3">
                  ...
                  <button
                    ...
                    className="flex-1 py-3 rounded-xl font-bold text-white text-[14px] flex items-center justify-center gap-2"
                    style={{
                      background: "linear-gradient(135deg,#14a8b5,#0d7377)",
                    }}
                  >
                ...
                <div>
                  ...
                  {isCustomPrice && (
                    <p
                      ...
                      style={{ color: "#64748b" }}
                    >
                      Price is set by the clinic — enter the amount agreed with
                      your doctor.
                    </p>
                  )}
                  <input
                    ...
                    style={{
                      background:
                        method === "full-usdc" && !isCustomPrice
                    ...
                    onFocus={(e) => {
                      if (method !== "full-usdc" || isCustomPrice)
                  ...
                  {method === "partial-usdc" &&
                    ...
                    parseFloat(usdcAmount) < displayTotal && (
                      <p
                        className="text-center text-[11px] mt-1.5 font-medium"
                        style={{ color: "#6366f1" }}
                      >
                        $
                        {(
                        ...
                        remaining — doctor will record any cash balance
                      </p>
                    )}
                </div>
                <div className="flex gap-3">
                  <button
                    onClick={() => setStep("usdc-connect")}
                    className="flex-1 py-2.5 rounded-xl text-[13px] font-semibold"
                    style={{ background: "#f1f5f9", color: "#64748b" }}
                  >
                    Back
                  ...
                  <button
                    ...
                    style={{
                      background: "linear-gradient(135deg,#14a8b5,#0d7377)",
                      opacity:
                        !usdcAmount || parseFloat(usdcAmount) <= 0 ? 0.5 : 1,
                    }}
                  >
              ...
              <div className="flex flex-col items-center gap-4 py-8">
                <div
                  ...
                  style={{
                    ...
                    border: "2px solid rgba(13,115,119,0.20)",
                  }}
                >
                  <Loader2
            ...
            {step === "error" && (
              <div className="flex flex-col items-center gap-4 py-6">
                ...
                <div className="text-center">
                  ...
                  <p
                    className="text-[12px] mt-1 max-w-[280px] leading-relaxed"
                    style={{ color: "#64748b" }}
                  >
                    {errorMsg}
                  </p>
