...
import { CalendarDays, Mic, Phone, Sparkles, X } from "lucide-react";
...
export function VoiceWidget({
  ...
  const { connect, disconnect, toggleMute, resumeAfterConfirmation, isMuted } =
    useRealtimeVoice({
      businessId,
      onConversationEnd: () => {},
    });
  ...
  const isConnected = ["connected", "listening", "speaking"].includes(
      ...
      <AnimatePresence mode="wait">
        {isOpen ? (
          <motion.div
            ...
            className="w-[340px] rounded-2xl overflow-hidden relative"
            style={{
          ...
          >
            {/* AI-triggered appointment confirmation overlay */}
            {pendingAppointment && (
              <AppointmentConfirmForm
                pending={pendingAppointment}
                primaryColor={primaryColor}
                onConfirmed={(callId, result) => {
                  resumeAfterConfirmation(callId, result);
                }}
              ...
              {(["call", "book"] as const).map((tab) => (
                <button
                  ...
                  style={{
                    ...
                    color: activeTab === tab ? primaryColor : "#475569",
                    cursor: "pointer",
                    fontFamily: "inherit",
                  }}
                >
                  ...
                  <span>{tab === "call" ? "Call AI" : "Book"}</span>
                </button>
              ))}
            </div>
              ...
              <>
                ...
                <div className="px-5 py-5 flex flex-col items-center gap-4">
                  {connectionState.status === "idle" && (
                    ...
                    >
                      {greeting ||
                        "Talk to our AI receptionist — ask about services, hours, or book an appointment"}
                    </p>
                  )}
                  <VoiceOrb
