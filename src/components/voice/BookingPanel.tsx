...
function getPriceStr(svc: Service): string {
  if (svc.price_type === 'fixed' && svc.price_min != null) return `$${svc.price_min}`;
  if (svc.price_type === 'range' && svc.price_min != null && svc.price_max != null)
    return `$${svc.price_min}–$${svc.price_max}`;
  if (svc.price_type === 'starting_at' && svc.price_min != null) return `From $${svc.price_min}`;
    ...
    <div className="flex items-center gap-1.5 mb-1.5">
      {([1, 2, 3, 4] as Step[]).map((n) => (
        <div
          ...
          style={{ background: n === current ? color : n < current ? `${color}80` : '#2a3f4d' }}
        />
      ))}
    </div>
  );
...
export function BookingPanel({ businessId, color }: BookingPanelProps) {
  ...
  useEffect(() => {
    ...
    setSlots([]);
    setClosedDay(false);
    fetch(`/api/widget/slots?businessId=${encodeURIComponent(businessId)}&date=${encodeURIComponent(selectedDate)}`)
      .then(r => r.json())
      .then(data => {
        if (data.closed) { setClosedDay(true); return; }
        setSlots(data.slots || []);
      })
      .catch(() => setSlots([]))
  ...
  if (selectedTimeKey && selectedService?.duration_minutes) {
    ...
    const endDt = new Date(startDt.getTime() + selectedService.duration_minutes * 60000);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formName || !formEmail || !formDob || !selectedTimeKey) {
      setSubmitError('Please fill in name, email, date of birth. Select a date and time.');
      return;
    }
    setSubmitting(true);
    setSubmitError(null);
  ...
  const stepHeader = (title: string, subtitle?: string) => (
    <div className="px-3.5 pt-3 pb-2" style={{ borderBottom: '1px solid rgba(128,128,128,0.12)' }}>
      <StepDots current={step} color={color} />
      <div className="text-[13px] font-bold" style={{ color: '#94a3b8' }}>{title}</div>
      {subtitle && <div className="text-[11px] mt-0.5" style={{ color: '#475569' }}>{subtitle}</div>}
    </div>
  );

  const stepNav = (onBack: (() => void) | null, onNext: (() => void) | null, nextDisabled?: boolean, isSubmit?: boolean) => (
    <div className="flex justify-between items-center px-3.5 pb-3 pt-2">
      ...
      {onNext !== null && (
        <button
          ...
          style={{ padding: '7px 18px', borderRadius: 9, border: 'none', background: color, fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}
        >
          {isSubmit && submitting ? <><Loader2 className="w-3.5 h-3.5 animate-spin" />Booking…</> : isSubmit ? 'Request Appointment' : 'Next'}
        </button>
      )}
    </div>
  );
  ...
  if (step === 1) {
    return (
      <div>
        ...
        <div className="flex flex-col gap-1.5 px-3.5 py-2 max-h-60 overflow-y-auto" style={{ scrollbarWidth: 'thin' }}>
          {loadingServices ? (
            <div className="flex items-center justify-center gap-2 py-6">
              <Loader2 className="w-4 h-4 animate-spin" style={{ color: '#475569' }} />
              <span className="text-[12px]" style={{ color: '#475569' }}>Loading…</span>
            </div>
          ) : services.length === 0 ? (
            <div className="text-center text-[12px] py-5" style={{ color: '#475569' }}>
              No services available. Please contact us directly.
            </div>
          ) : services.map(svc => {
            const priceStr = getPriceStr(svc);
            const durStr = svc.duration_minutes ? `${svc.duration_minutes} min` : '';
            ...
            return (
              ...
              >
                <div className="text-[12px] font-semibold" style={{ color: '#e2e8f0' }}>{svc.name}</div>
                {(durStr || priceStr) && (
                  <div className="flex items-center gap-1.5 mt-0.5 text-[11px]" style={{ color: '#64748b' }}>
                    {durStr && <span>{durStr}</span>}
                    {durStr && priceStr && <span style={{ opacity: 0.4 }}>·</span>}
      ...
      <div>
        ...
        <div>
          ...
          <div className="grid grid-cols-7 gap-0.5 px-2.5 pb-1">
            {DAY_NAMES.map(d => (
              <div key={d} className="text-center py-0.5 text-[9px] font-semibold uppercase tracking-wider"
            ...
            {Array.from({ length: daysInMonth }, (_, i) => {
              ...
              return (
                <button
                  ...
                  type="button"
                  disabled={isPast}
                  onClick={() => { setSelectedDate(dateStr); setSelectedTime(null); setSelectedTimeKey(null); }}
        ...
        {stepNav(() => setStep(1), () => { if (selectedDate) setStep(3); }, !selectedDate)}
      </div>
    );
  }
  ...
  if (step === 3) {
    return (
      <div>
        {stepHeader('Pick a Time', selectedDate ? formatDateLabel(selectedDate) : undefined)}
        <div className="px-3.5 py-2 min-h-[120px]">
          {loadingSlots ? (
            <div className="flex items-center justify-center gap-2 py-8">
              <Loader2 className="w-4 h-4 animate-spin" style={{ color: '#475569' }} />
              <span className="text-[12px]" style={{ color: '#475569' }}>Loading slots…</span>
            </div>
          ) : closedDay ? (
            <div className="text-center text-[12px] py-6" style={{ color: '#475569' }}>
            ...
            <div className="grid grid-cols-3 gap-1.5 max-h-[200px] overflow-y-auto" style={{ scrollbarWidth: 'thin' }}>
              {slots.map(time => {
                ...
                return (
                  <button
                    key={time}
                    type="button"
                    onClick={() => { setSelectedTime(time); setSelectedTimeKey(displayTimeToKey(selectedDate!, time)); }}
        ...
        {stepNav(() => setStep(2), () => { if (selectedTime) setStep(4); }, !selectedTime)}
      </div>
    );
  }
  ...
  return (
    <div>
      {success ? (
        <div className="flex flex-col items-center justify-center py-10 px-5 gap-1.5 text-center" style={{ minHeight: 200 }}>
          <CheckCircle className="w-10 h-10 mb-1" style={{ color }} />
          <div className="text-[14px] font-bold" style={{ color: '#f1f5f9' }}>Appointment Requested!</div>
          <div className="text-[11px] mt-0.5" style={{ color: '#64748b' }}>We will confirm for</div>
          <div className="text-[13px] font-semibold" style={{ color: '#94a3b8' }}>
            {selectedDate ? formatDateLabel(selectedDate) : ''} at {selectedTime || ''}{endTimeLabel}
          </div>
        ...
        <>
          <div className="px-3.5 pt-3 pb-2" style={{ borderBottom: '1px solid rgba(128,128,128,0.12)' }}>
            ...
            <div className="text-[13px] font-bold mb-1.5" style={{ color: '#94a3b8' }}>Your Details</div>
            <div className="flex flex-wrap gap-1.5">
              ...
              {selectedDate && selectedTime && (
                <span className="text-[10px] px-2 py-0.5 rounded-full font-medium"
                  style={{ background: 'rgba(255,255,255,0.06)', color: '#94a3b8' }}>
                  {formatDateLabel(selectedDate)} · {selectedTime}{endTimeLabel}
                </span>
              )}
            </div>
          </div>
          ...
          >
            ...
            <div className="flex gap-2">
              <div className="flex-1 min-w-0">
                ...
                <input value={formEmail} onChange={e => setFormEmail(e.target.value)}
                  type="email" placeholder="jane@example.com" required style={inputStyle} />
              </div>
              <div className="flex-1 min-w-0">
            ...
            <div>
              ...
              <input value={formDob} onChange={e => setFormDob(e.target.value)}
                type="date" required style={{ ...inputStyle, colorScheme: 'dark' } as React.CSSProperties} />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                style={{ ...inputStyle, resize: 'none', lineHeight: 1.4 } as React.CSSProperties}
              />
            </div>
            ...
            <div className="flex justify-between items-center mt-1 mb-1">
              ...
              <button
                ...
                style={{ padding: '7px 18px', borderRadius: 9, border: 'none', background: color, fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}
              >
                {submitting ? <><Loader2 className="w-3.5 h-3.5 animate-spin" />Booking…</> : 'Request Appointment'}
              </button>
            </div>
          </form>
        </>
