...
import {
  LayoutDashboard, Bot, Calendar, MessageSquare, ClipboardList,
  HelpCircle, Code2, BarChart3, Settings, LogOut, Activity,
  HeartPulse, Bell, Headphones, Users, CalendarDays, Globe,
} from 'lucide-react';
...
const navGroups = [
  {
    ...
    items: [
      ...
      { href: '/dashboard/conversations', label: 'Call Log',        icon: MessageSquare },
      { href: '/dashboard/appointments',          label: 'Appointments',   icon: Calendar },
      { href: '/dashboard/appointments/schedule', label: 'Schedule',       icon: CalendarDays },
...
export function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  return (
    <>
      {isOpen && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm z-20 lg:hidden" onClick={onClose} />
      )}
      ...
      <aside className={cn(
        ...
        style={{
          background: 'linear-gradient(180deg, #072b2e 0%, #0a3d40 35%, #0d5257 70%, #0d6b70 100%)',
        ...
        {/* Logo */}
        <div className="relative z-10 flex items-center gap-3 h-16 px-5" style={{ borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
          ...
          <div className="flex flex-col leading-none">
            ...
            <span className="text-[10px] font-medium mt-0.5" style={{ color: 'rgba(255,255,255,0.45)' }}>AI Voice Platform</span>
          </div>
        </div>

        {/* Practice badge */}
        {business && (
          <div className="relative z-10 px-4 py-3" style={{ borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
            <div className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl"
              ...
              <div className="w-7 h-7 rounded-lg flex items-center justify-center text-[11px] font-bold text-white flex-shrink-0"
                style={{ background: 'rgba(34,196,208,0.30)', border: '1px solid rgba(34,196,208,0.40)' }}>
                {business.name.substring(0, 2).toUpperCase()}
              </div>
              <div className="flex-1 min-w-0">
                <div className="text-[12px] font-semibold text-white truncate leading-tight">{business.name}</div>
                <div className="text-[10px] truncate mt-0.5" style={{ color: 'rgba(255,255,255,0.45)' }}>
                  {business.city || 'Healthcare Practice'}
                </div>
              </div>
              <Activity className="w-3.5 h-3.5 flex-shrink-0 animate-heartbeat" style={{ color: '#22c4d0' }} />
            </div>
          </div>
        )}
                ...
                {group.items.map((item) => {
                  ...
                  return (
                    <Link key={item.href} href={item.href} onClick={onClose}
                      className={cn('relative flex items-center gap-2.5 px-3 py-2 rounded-xl transition-all duration-150 group',
                        active ? 'text-white font-semibold' : 'font-medium'
                      )}
                      style={{ color: active ? undefined : 'rgba(255,255,255,0.50)' }}>
                      {active && (
                        <motion.div
                      ...
                      )} style={{ color: active ? undefined : 'rgba(255,255,255,0.40)' }} />
        ...
        {/* Sign out */}
        <div className="relative z-10 px-3 py-3" style={{ borderTop: '1px solid rgba(255,255,255,0.08)' }}>
