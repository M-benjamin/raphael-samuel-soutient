...
type AppWithService = Appointment & {
  service?: {
    name: string;
...
export default function PortalAppointmentsPage() {
  ...
  const [payingAppointment, setPayingAppointment] =
  ...
  const loadAppointments = async (email: string) => {
    ...
    const { data } = await supabase
      .from("appointments")
      .select(
        "*, service:services(name, duration_minutes, price_min, price_max, price_type)",
      )
  ...
  const submitReschedule = async (id: string) => {
    ...
    try {
      ...
      setAppointments((prev) =>
        prev.map((a) =>
          a.id === id
            ? { ...a, scheduled_at: picked.toISOString(), status: "pending" }
            : a,
        ),
      );
      ...
      {payingAppointment && businessId && (
        <PaymentModal
          ...
          onPaid={(txHash, amountPaid, payMethod) => {
            setAppointments((prev) =>
              prev.map((a) =>
                a.id === payingAppointment.id
                  ? {
                      ...a,
                      payment_status:
                        payMethod === "partial" ? "partial" : "paid",
      ...
      <main className="max-w-2xl mx-auto px-4 py-8">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center py-24 gap-3 text-center">
            <HeartPulse
              className="w-12 h-12"
              style={{ color: "rgba(13,115,119,0.25)" }}
            />
            <p className="text-[16px] font-bold" style={{ color: "#0a2e30" }}>
              No appointments yet
            </p>
            <p className="text-[13px] max-w-xs" style={{ color: "#64748b" }}>
              Your appointments will appear here after booking via our AI
              receptionist.
            </p>
          </div>
        ) : (
...
function AppointmentCard({
  ...
  const canReschedule =
    isUpcoming && !["completed", "cancelled", "no_show"].includes(appt.status);
  const canCancel =
    isUpcoming && !["completed", "cancelled", "no_show"].includes(appt.status);
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-3 mt-1 flex-wrap">
              <span
                ...
                style={{ color: "#64748b" }}
              >
                <Clock className="w-3 h-3" style={{ color: "#0d7377" }} />{" "}
                {timeStr}
              </span>
              <span
                className="flex items-center gap-1 text-[12px]"
        ...
        {(canReschedule || canCancel || appt.payment_status === "unpaid") && (
          ...
          >
            ...
            {canReschedule && !isRescheduling && (
              <button
                type="button"
                onClick={() => onStartReschedule(appt.id)}
                className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-[12px] font-semibold transition-all hover:scale-[1.02]"
                style={{
            ...
            >
              ...
              >
                ...
                <div className="flex gap-2">
                  ...
                  >
                    {rescheduling ? (
                      ...
                      <>
                        <CalendarClock className="w-3.5 h-3.5" /> Confirm
                        Reschedule
                      </>
                    )}
                  </button>
                ...
                <p className="text-[10px]" style={{ color: "#94a3b8" }}>
                  Status will reset to Pending — the clinic will re-approve your
                  new time.
                </p>
              </div>
            </motion.div>
                ...
                <div className="flex items-start gap-2 mb-3">
                  ...
                  <div>
                    ...
                    <p
                      ...
                      style={{ color: "#94a3b8" }}
                    >
                      This cannot be undone. You'll receive a confirmation
                      email.
                    </p>
                  </div>
                </div>
      ...
      <AnimatePresence>
        {expanded && (
          ...
          >
            ...
            >
              <p
                className="text-[10px] font-bold uppercase tracking-widest pt-3"
                style={{ color: "#0d7377" }}
              >
                Appointment Details
              </p>
              ...
              {/* {canJoinCall(appt) && ( */}
              {/* <Link href={`/call/${appt.id}`} className="mt-2 inline-block"> */}
