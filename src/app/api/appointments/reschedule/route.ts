...
export async function PATCH(req: NextRequest) {
  try {
    const { appointmentId, newScheduledAt, requestedBy } = await req.json() as {
    ...
    if (!appointmentId || !newScheduledAt) {
      return NextResponse.json({ error: 'appointmentId and newScheduledAt are required' }, { status: 400 });
    }

    if (isNaN(new Date(newScheduledAt).getTime())) {
      return NextResponse.json({ error: 'Invalid newScheduledAt date' }, { status: 400 });
    }
    ...
    const durationMinutes: number = (appt.service as { duration_minutes?: number } | null)?.duration_minutes ?? appt.duration_minutes ?? 60;
    ...
    const biz = business as { name: string; email: string | null; phone: string | null } | null;
    ...
    await supabase.from('notifications').insert({
      ...
      metadata: { appointment_id: appointmentId, requested_by: requestedBy ?? 'owner' },
    });
...
<table width="560" cellpadding="0" cellspacing="0" style="margin:auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(13,115,119,0.10);">
  ...
  <tr><td style="background:#f8fafc;padding:16px 36px;text-align:center;border-top:1px solid #e2e8f0;">
    <p style="margin:0;font-size:11px;color:#94a3b8;">Sent by <strong>${biz?.name}</strong> via MediCall AI · ID: ${appointmentId}</p>
  </td></tr>
</table></body></html>`;
