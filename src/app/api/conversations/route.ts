...
export async function PATCH(req: NextRequest) {
  try {
    ...
    // Whitelist allowed update fields — never spread raw request body into DB update
    const finalUpdates: Record<string, unknown> = {};
    if (updates.status !== undefined) finalUpdates.status = updates.status;
    if (updates.duration_seconds !== undefined) finalUpdates.duration_seconds = updates.duration_seconds;
    if (updates.appointment_booked !== undefined) finalUpdates.appointment_booked = updates.appointment_booked;
    if (updates.callback_requested !== undefined) finalUpdates.callback_requested = updates.callback_requested;
    ...
    return NextResponse.json({ error: 'Failed to update conversation' }, { status: 500, headers: CORS });
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: 'Failed to save message' }, { status: 500, headers: CORS });
  }
}
